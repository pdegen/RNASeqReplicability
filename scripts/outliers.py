...
def get_outlier_patients(df, method, outname="", outpath="", **kwargs) -> list:
    ...
    else: raise Exception(f"Outlier method {method} not implemented")
...
def jackknife_wrapper(df, outname, path, DEA="edgeR", FDR=0.01, overwrite=False, include_full=True, skip_cols="skip_none", **DEA_kwargs) -> int:
    ...
    return jackknife_paired(df_r, outname, path, overwrite=overwrite, include_full=include_full, DEA=DEA, skip_cols=skip_cols, cols_to_keep=cols_to_keep)[0]
...
def iterative_jackknife(df, outname, path, DEA="edgeR", FDR=0.01, overwrite=False, max_removed_frac=1, efficient=False, 
    ...
    while len(outliers) > 0 and total_outliers < max_removable_patients:
        ...
        # Run jackknife
        exit_status = jackknife_wrapper(df, outname=name, path=path, DEA=DEA, FDR=FDR, overwrite=overwrite, skip_cols=skip_cols, **DEA_kwargs)
