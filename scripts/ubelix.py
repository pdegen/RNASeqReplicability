        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith(time_str):
                ...
                if outlier_method == "jk":
                    ...
                    max_time = 1+N/8
                else: raise Exception(f"Outlier method {outlier_method} not implemented")
...
def adjust_gsea_job_time(N,libraries, gsea_method, gsea_script_path, just_testing=False) -> int:
    ...
    if gsea_method.startswith("gseapy"):
        ...
        max_time = 4 + N//7
    else: raise Exception(f"Enrichment method {gsea_method} not implemented")
        ...
        for DEA in config_params["DEA_methods"]:
            ...
            for out in config_params["outlier_methods"]:
                ...
                for cohort in range(1,n_cohorts+1):
                    ...
                    if outpath_c.exists():
                        if not (Path(f"{outpath_c}/tab.{out}.{DEA}.{param_set}.csv").exists() or Path(f"{outpath_c}/tab.{out}.{DEA}.{param_set}.feather").exists()):
        ...
        for DEA in config_params["DEA_methods"]:
            ...
            for out in config_params["outlier_methods"]:
                ...
                for gsea_method in config_params["gsea_methods"]:
                            ...
                            if gseapath.exists():
                                ...
                                for library in libraries:
                                    ...
                                    if "clusterORA" in gsea_method:
                                        g=glob.glob(f"{outpath_c}/gsea/{gsea_method}*.{library}.{DEA}.{out}.{gsea_param_set}.feather")
                                    else:
                                        g=glob.glob(f"{outpath_c}/gsea/{gsea_method}.{library}.{DEA}.{out}.{gsea_param_set}.feather")
                                    
                                    if len(g) >= 1 and gsea_method in ["gseapy","gseapy_s2n"]: gsea_results_exist = True
                                    elif len(g) >= 1 and gsea_method in ["clusterORA","clusterORA_lfc"]: gsea_results_exist = True
                                    else: gsea_results_exist = False
                    ...
                    # Send the jobs
                    max_time = adjust_gsea_job_time(N=N,libraries=libraries, gsea_method=gsea_method, gsea_script_path=gsea_script_path)
