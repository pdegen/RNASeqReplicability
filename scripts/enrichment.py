...
def run_gseapy(prepared_tab, library, outpath, threads=4, permutation_num=100, file_id="",ranking="logFC", min_size=15, max_size=500):
    """
    ...
    terms = terms[["Name","Term","Term ID","ES", "NES", "NOM p-val", "FDR q-val", "FWER p-val", "Tag %", "Gene %" ,"Lead_genes"]]
    ...
    terms.to_feather(f"{outpath}/gseapy{rnk}.{library}{file_id}.feather" if file_id != "" else f"{outpath}/gseapy{rnk}.{library}.feather")
    os.system(f"rm {outpath}/gseapy.gene_set.prerank.report.csv")
    
    return pre_res

def run_gseapy_libraries(prepared_tab, gseapath, libraries, overwrite_all_gsea, file_id="", permutation_num=100, save_full_results=False, threads=4,ranking="logFC", min_size=15, max_size=500):
    rnk = "_s2n" if ranking == "|S2N|" else ""
    for library in libraries: 
        ...
        if not Path(gsea_out).is_file() or overwrite_all_gsea:
            ...
            gsea_results = run_gseapy(prepared_tab, library, gseapath, permutation_num=permutation_num, 
                                      file_id=file_id, threads=threads, ranking=ranking, min_size=min_size, max_size=max_size)
...
def run_clusterORA(degs, universe, file_id, go_ont, prefix="", overwrite=False, use_internal_data=False, internal_data_path = "", minGSSize=15, maxGSSize=500, **unused):
    """
    ...
    clusterORA(degs, universe, file_id, go_ont, prefix=prefix, overwrite=overwrite, minGSSize=minGSSize, maxGSSize=maxGSSize,
...
def run_gsea(tab, gseapath, libraries, gsea_method, overwrite=False, file_id="", **gsea_kwargs):
    ...
    else: raise Exception(f"{gsea_method} not implemented!")
